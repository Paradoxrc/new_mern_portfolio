...
const Skills = () => {
  ...
  // If skills data is unavailable, return null or a loading state
        ...
        <SkillsContainer>
          {skills.map((skill) => (
            <Skill key={skill.id}> {/* Use a unique key for each skill */}
              <SkillTitle>{skill.title}</SkillTitle> {/* Access each skill's title */}
              <SkillList>
                {skill.skills.map((item) => (
                  <SkillItem key={item.name}> {/* Use a unique key for each item */}
